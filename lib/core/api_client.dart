...
import 'package:app_jobfind/main.dart';
import 'constants.dart';

class ApiClient {
  late final Dio dio;

  // Cờ tránh refresh nhiều lần đồng thời
  ...
  ApiClient() {
    ...
    dio.interceptors.add(
      InterceptorsWrapper(
        ...
        // ── Xử lý lỗi – quan trọng: auto-refresh khi 401 ──
          ...
          if (error.response?.statusCode == 401 && !_isRefreshing) {
            ...
            try {
              ...
              final refreshResponse = await dio.post(
                ...
                options: Options(headers: {'Authorization': ''}), // bỏ token cũ
              );
  ...
  Future<void> _forceLogout(SharedPreferences prefs) async {
    ...
    // Navigate về SplashScreen/LoginScreen qua GlobalKey
  ...
  /// Xử lý chung Response từ backend
  ...
  Future<dynamic> uploadFile(String path, FormData formData) async {
    try {
      final response = await dio.post(
        path,
        data: formData,
        // Set contentType explicitly to multipart/form-data
