...
class ChatService {
  ...
  Future<ChatConversationDto> getOrCreateConversation(int recipientId, int? jobPostId) async {
  ...
  Future<List<ChatConversationDto>> getConversations({int pageNumber = 1, int pageSize = 20}) async {
  ...
  Future<List<ChatMessageDto>> getMessages(int conversationId, {int pageNumber = 1, int pageSize = 50}) async {
    final data = await _apiClient.get(
