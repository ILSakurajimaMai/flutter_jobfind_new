...
class _CreateJobPostScreenState extends ConsumerState<CreateJobPostScreen> {
  ...
  static const _workTypes = ['Part-time', 'Full-time', 'Freelance', 'Internship'];
  static const _salaryPeriods = ['Hourly', 'Daily', 'Weekly', 'Monthly'];
  static const _salaryPeriodLabels = ['Giờ', 'Ngày', 'Tuần', 'Tháng'];
  static const _categories = [
    'Công nghệ thông tin', 'Marketing', 'Bán hàng', 'Kế toán',
    'Nhà hàng / Khách sạn', 'Kho vận / Giao vận', 'Phục vụ',
    'Giáo dục', 'Y tế', 'Xây dựng', 'Khác',
  ];
  ...
  @override
  void dispose() {
    _titleCtrl.dispose(); _locationCtrl.dispose(); _positionsCtrl.dispose();
    _descCtrl.dispose(); _reqCtrl.dispose(); _benefitsCtrl.dispose();
    _salaryMinCtrl.dispose(); _salaryMaxCtrl.dispose(); _skillInputCtrl.dispose();
    super.dispose();
  }
  ...
  void _addSkill() {
    ...
    if (skill.isNotEmpty && !_skills.contains(skill)) {
      setState(() { _skills.add(skill); _skillInputCtrl.clear(); });
    }
  }
  ...
  void _addShift() {
    setState(() => _shifts.add(JobShiftRequest(dayOfWeek: 1, startTime: '08:00', endTime: '17:00')));
  }
    ...
    if (mounted) {
      if (success) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(_isEditing ? '✅ Cập nhật thành công!' : '✅ Đăng tin thành công!'),
      ...
      body: Form(
        ...
        child: Theme(
          data: Theme.of(context).copyWith(
            colorScheme: Theme.of(context).colorScheme.copyWith(primary: const Color(0xFF14003E)),
          ),
          child: Stepper(
            ...
            controlsBuilder: (ctx, details) => _buildStepControls(ctx, details, state.isSaving),
            steps: [
              Step(
                title: const Text('Thông tin cơ bản', style: TextStyle(fontWeight: FontWeight.w600)),
              ...
              Step(
                title: const Text('Mô tả công việc', style: TextStyle(fontWeight: FontWeight.w600)),
              ...
              Step(
                title: const Text('Mức lương', style: TextStyle(fontWeight: FontWeight.w600)),
              ...
              Step(
                title: const Text('Kỹ năng & Ca làm việc', style: TextStyle(fontWeight: FontWeight.w600)),
  ...
  Widget _buildStepControls(BuildContext ctx, ControlsDetails details, bool isSaving) {
          ...
          Expanded(
            child: ElevatedButton(
              ...
              child: isSaving && isLast
                  ...
                  : Text(isLast ? (_isEditing ? 'Lưu thay đổi' : 'Đăng tin') : 'Tiếp theo',
  ...
  Widget _buildStep1() {
    return Column(
      ...
      children: [
        ...
        _field(_titleCtrl, hint: 'Ví dụ: Nhân viên phục vụ part-time', isRequired: true),
        const SizedBox(height: 16),

        _label('Ngành nghề / Danh mục'),
        _buildDropdown(_category, _categories, 'Chọn ngành nghề', (v) => setState(() => _category = v)),
        const SizedBox(height: 16),

        _label('Loại hình công việc'),
        Wrap(
          spacing: 8, runSpacing: 8,
          children: _workTypes.map((t) => ChoiceChip(
            label: Text(t),
            selected: _workType == t,
            onSelected: (_) => setState(() => _workType = t),
            selectedColor: const Color(0xFF14003E),
            labelStyle: TextStyle(color: _workType == t ? Colors.white : Colors.black87, fontWeight: FontWeight.w500),
        ...
        Row(children: [
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            _field(_positionsCtrl, hint: '1', keyboardType: TextInputType.number),
          ])),
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                child: Row(children: [
                  ...
                  Text(
                    _deadline != null ? '${_deadline!.day}/${_deadline!.month}/${_deadline!.year}' : 'Chọn ngày',
                    style: TextStyle(color: _deadline != null ? Colors.black87 : Colors.grey.shade400, fontSize: 13),
                  ),
                ]),
              ),
            ),
        ...
        Container(
          ...
          child: Row(children: [
            const Icon(Icons.local_fire_department_outlined, color: Color(0xFFFF6B35)),
            const SizedBox(width: 8),
            const Expanded(child: Text('Tuyển gấp', style: TextStyle(fontWeight: FontWeight.w600))),
            Switch(
              value: _isUrgent,
  ...
  Widget _buildStep2() {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      _field(_descCtrl, hint: 'Mô tả chi tiết công việc, nhiệm vụ cần thực hiện...', maxLines: 5, isRequired: true),
      const SizedBox(height: 16),
      _label('Yêu cầu ứng viên'),
      _field(_reqCtrl, hint: 'Kinh nghiệm, bằng cấp, tố chất cần có...', maxLines: 4),
      const SizedBox(height: 16),
      _label('Quyền lợi / Phúc lợi'),
      _field(_benefitsCtrl, hint: 'Lương thưởng, bảo hiểm, du lịch, đào tạo...', maxLines: 4),
    ]);
  }
  ...
  Widget _buildStep3() {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Wrap(
        spacing: 8, runSpacing: 8,
        children: List.generate(_salaryPeriods.length, (i) => ChoiceChip(
          label: Text(_salaryPeriodLabels[i]),
          selected: _salaryPeriod == _salaryPeriods[i],
          onSelected: (_) => setState(() => _salaryPeriod = _salaryPeriods[i]),
          selectedColor: const Color(0xFF14003E),
          labelStyle: TextStyle(
            color: _salaryPeriod == _salaryPeriods[i] ? Colors.white : Colors.black87,
      ...
      Row(children: [
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          _field(_salaryMinCtrl, hint: '3000000', keyboardType: TextInputType.number),
        ])),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          _label('Lương tối đa (VNĐ)'),
          _field(_salaryMaxCtrl, hint: '6000000', keyboardType: TextInputType.number),
        ])),
      ]),
      ...
      Container(
        ...
        child: Row(children: [
          const Icon(Icons.info_outline, size: 16, color: Color(0xFF14003E)),
          const SizedBox(width: 8),
          const Expanded(child: Text('Để trống nếu lương thỏa thuận', style: TextStyle(fontSize: 12, color: Color(0xFF14003E)))),
        ]),
      ),
    ]);
  }
  ...
  Widget _buildStep4() {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Row(children: [
        Expanded(child: _field(_skillInputCtrl, hint: 'Nhập kỹ năng rồi nhấn +')),
        const SizedBox(width: 8),
        GestureDetector(
          onTap: _addSkill,
          child: Container(
            padding: const EdgeInsets.all(14),
            decoration: BoxDecoration(color: const Color(0xFF14003E), borderRadius: BorderRadius.circular(12)),
      ...
      Row(children: [
        const Text('Ca làm việc', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: Color(0xFF14003E))),
        const Spacer(),
        TextButton.icon(
          onPressed: _addShift,
          icon: const Icon(Icons.add, size: 18),
          label: const Text('Thêm ca'),
          style: TextButton.styleFrom(foregroundColor: const Color(0xFF14003E)),
        ),
      ]),
      ..._shifts.asMap().entries.map((entry) => _buildShiftRow(entry.key, entry.value)),
      if (_shifts.isEmpty)
        const Padding(
          padding: EdgeInsets.symmetric(vertical: 8),
          child: Text('(Không bắt buộc)', style: TextStyle(color: Colors.grey, fontSize: 12)),
        ),
    ]);
  }
  ...
  Widget _buildShiftRow(int index, JobShiftRequest shift) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          const Text('Thứ:', style: TextStyle(fontSize: 12, color: Colors.grey)),
          const SizedBox(width: 8),
          Expanded(
            child: SingleChildScrollView(
              ...
              child: Row(
                children: List.generate(7, (d) => Padding(
                  ...
                  child: ChoiceChip(
                    label: Text(_dayLabels[d], style: const TextStyle(fontSize: 11)),
                    selected: shift.dayOfWeek == d,
                    onSelected: (_) => setState(() => shift.dayOfWeek = d),
                    selectedColor: const Color(0xFF14003E),
                    labelStyle: TextStyle(color: shift.dayOfWeek == d ? Colors.white : Colors.black87),
                    backgroundColor: Colors.grey.shade100,
          ...
          IconButton(icon: const Icon(Icons.delete_outline, color: Colors.red, size: 20), onPressed: () => _removeShift(index)),
        ]),
        const SizedBox(height: 8),
        Row(children: [
          Expanded(child: _buildTimePicker('Bắt đầu', shift.startTime, (v) => setState(() => shift.startTime = v))),
          const Padding(padding: EdgeInsets.symmetric(horizontal: 8), child: Text('–', style: TextStyle(fontWeight: FontWeight.bold))),
          Expanded(child: _buildTimePicker('Kết thúc', shift.endTime, (v) => setState(() => shift.endTime = v))),
        ]),
      ]),
    );
  }

  Widget _buildTimePicker(String label, String value, Function(String) onChanged) {
    return GestureDetector(
      onTap: () async {
        ...
        final picked = await showTimePicker(context: context, initialTime: initialTime,
          builder: (ctx, child) => Theme(
            data: Theme.of(ctx).copyWith(colorScheme: const ColorScheme.light(primary: Color(0xFF14003E))),
            child: child!,
          ));
      ...
      child: Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(children: [
            const Icon(Icons.access_time_outlined, size: 14, color: Color(0xFF14003E)),
            const SizedBox(width: 4),
            Text(value, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 13, color: Color(0xFF14003E))),
          ]),
        ]),
      ),
    );
  ...
  Widget _field(TextEditingController ctrl, {String? hint, int maxLines = 1, TextInputType? keyboardType, bool isRequired = false}) {
    return Container(
      decoration: BoxDecoration(
        color: Colors.white, borderRadius: BorderRadius.circular(12),
        boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: 0.03), blurRadius: 8)],
      ...
      child: TextFormField(
        ...
        validator: isRequired ? (v) => (v == null || v.trim().isEmpty) ? 'Trường này bắt buộc' : null : null,
      ),
    );
  }
